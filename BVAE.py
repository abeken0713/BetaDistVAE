...
class BVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            z = mr
        abg = self.dec(z)   # alpha, beta generated
        ag, bg = torch.chunk(abg, 2, 1)
        return mr, sr, ag, bg

    def loss(self, x):
        # x: (batch_size, n_dim)
        mr, sr, ag, bg = self.forward(x)
        # mr, sr: (batch_size, hid_dim)
        # mg: (batch_size, n_dim)
        nll = self._beta_nll(x, ag, bg)    # (batch_size,)
        kld = self._gaussian_kld(mr, sr)    # (batch_size,)
        out = nll + kld
        return torch.mean(out)

    def _beta_nll(self, x, a, b):
        # x, m: (batch_size, n_dim)
        out = (a - 1)*log_clip(x) + (b - 1)*log_clip(1 - x) - lbeta(a, b)
        return -out.sum(1)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        model.eval()
        _, _, a_pred, b_pred = model(X_test)
        # X_pred = (a_pred - 1)/(a_pred + b_pred - 2)   # Mode
        X_pred = a_pred/(a_pred + b_pred)   # Mean
        X_test = X_test.numpy()
        X_pred = X_pred.numpy()
